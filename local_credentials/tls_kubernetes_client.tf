...
resource "tls_private_key" "kubernetes-client" {
  algorithm   = data.terraform_remote_state.host.outputs.kubernetes_ca.algorithm
  ecdsa_curve = "P521"
}
...
resource "tls_locally_signed_cert" "kubernetes-client" {
  ...
  ca_private_key_pem = data.terraform_remote_state.host.outputs.kubernetes_ca.private_key_pem
