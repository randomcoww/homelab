...
resource "matchbox_group" "controller_0" {
  ...
  metadata {
    ...
    manifest_url  = "http://matchbox.svc.internal:58080/generic?manifest=controller"
    hyperkube_image = "gcr.io/google_containers/hyperkube:v1.10.3"

    tls_ca        = "${chomp(tls_self_signed_cert.root.cert_pem)}"
    tls_ca_key    = "${chomp(tls_private_key.root.private_key_pem)}"
    ...
    tls_controller_manager  = "${chomp(tls_locally_signed_cert.controller_manager.cert_pem)}"
    tls_controller_manager_key = "${chomp(tls_private_key.controller_manager.private_key_pem)}"
    tls_scheduler  = "${chomp(tls_locally_signed_cert.scheduler.cert_pem)}"
    tls_scheduler_key = "${chomp(tls_private_key.scheduler.private_key_pem)}"
