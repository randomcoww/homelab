resource "tls_private_key" "valkey" {
  for_each = {
    for _, member in local.members :
    member.name => member
  }

  algorithm   = var.ca.algorithm
  ecdsa_curve = "P521"
  rsa_bits    = 4096
}

resource "tls_cert_request" "valkey" {
  for_each = {
    for _, member in local.members :
    member.name => member
  }

  private_key_pem = tls_private_key.valkey[each.key].private_key_pem
  subject {
    common_name = each.value.name
  }
  ip_addresses = [
    "127.0.0.1",
  ]
  dns_names = [
    each.value.name,
    "${each.value.name}.${local.headless_service}",
    "${each.value.name}.${local.headless_service_fqdn}",
    var.service_hostname,
  ]
}

resource "tls_locally_signed_cert" "valkey" {
  for_each = {
    for _, member in local.members :
    member.name => member
  }

  cert_request_pem   = tls_cert_request.valkey[each.key].cert_request_pem
  ca_private_key_pem = var.ca.private_key_pem
  ca_cert_pem        = var.ca.cert_pem

  validity_period_hours = 8760
  early_renewal_hours   = 2160

  allowed_uses = [
    "key_encipherment",
    "digital_signature",
    "server_auth",
    "client_auth",
  ]
}

module "tls" {
  source  = "../../../modules/secret"
  name    = "${var.name}-tls"
  app     = var.name
  release = var.release
  data = merge({
    for _, member in local.members :
    "${member.name}-tls.crt" => tls_locally_signed_cert.valkey[member.name].cert_pem
    }, {
    for _, member in local.members :
    "${member.name}-tls.key" => tls_private_key.valkey[member.name].private_key_pem
    }, {
    "ca.crt" = var.ca.cert_pem
  })
}