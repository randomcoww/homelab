locals {
  ...
  extra_configs = merge(var.extra_configs, {
    ...
    # OAUTH_CLIENT_INFO_ENCRYPTION_KEY   = random_password.client-info-encryption-key.result # not working with MCP OIDC 2.1
    # OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = random_password.session-token-encryption-key.result # not working with MCP OIDC 2.1
  })
}
