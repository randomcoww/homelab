...
# write this to use from another terrafrom instance
resource "local_file" "matchbox_ca_cert" {
  content  = module.addons-pxeboot.ca.matchbox.cert_pem
  filename = "./output/certs/matchbox-ca.pem"
}

resource "local_file" "matchbox_client_cert" {
  content  = tls_locally_signed_cert.matchbox-client.cert_pem
  filename = "./output/certs/matchbox-cert.pem"
}

resource "local_file" "matchbox_client_key" {
  content  = tls_private_key.matchbox-client.private_key_pem
  filename = "./output/certs/matchbox-key.pem"
}